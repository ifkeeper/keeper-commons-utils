...
public class PostUtil {
     ...
     *    s 值为 60466175 时 36进制编码字符为 zzzzz, 也是最大五位最大允许值, 当 s 值大于 60466175 时
     *    timeCompress 值为6位.
     ...
     * <br>
     *
     * 场景: 当前系统有十亿+用户, 其中某些用户是大 V 用户, 该用户的帖子通常为热点数据, 有些
     * 用户可能会进行查阅 V 用户的历史帖子, 假设该用户每天发表 5+ 帖子, 一周就是 35+ 帖子.
     * 所以当热点用户较多时直接存储在 DB 中会存在很大程度的压力.
     * 因此可以将该用户的帖子进行分组, 这里按周进行分组(也可以自行按月分组). 每周的贴在都存在
     ...
     *    或者直接读取为 1570377600 (直接采用 1570377600 id 长度会响应的边长).
     *
